...
class CustomTypeDecoder:
    ...
    def _compiler_declaration(self) -> Compiler.Declaration:
        ...
        return Elm.customTypeWith(
            self.name, [g for g in self.generics], self.compiler_variants
        )
        ...
        match flag:
            ...
            case AliasFlag(vars=alias_vars):
                prepared_flags = _prepare_inline_flags(
                    flag,
                    ObjectDecoder("inlineToModel", 1, alias_vars or set()),
                    decoder_sig=decoder_sig,
                )
            case _:
                prepared_flags = _prepare_inline_flags(
                    flag,
                    ObjectDecoder("inlineToModel", 1, set()),
                    decoder_sig=decoder_sig,
                )
...
def _prepare_inline_flags(
    ...
    match flag:
        case AliasFlag(name=alias_name, obj=alias_flag, vars=alias_vars):
            alias_object_decoder = ObjectDecoder(alias_name, 1, alias_vars or set())
        ...
        case CustomTypeFlag(variants=v):
            ...
            next_object_decoder = ObjectDecoder(
                object_decoder.value, object_decoder.depth + 1, set()
            )
        ...
        try:
            ...
            match value_flag:
                case AliasFlag(name=alias_name, obj=alias_obj, vars=alias_vars):
                    object_decoder = ObjectDecoder(alias_name, 1, alias_vars or set())
                ...
                case ModelChoiceFieldFlag() as mcf:
                    mcf_flag = mcf.obj()
                    assert isinstance(mcf_flag, ObjectFlag)
                    decoder = ObjectDecoder(key, depth, set(), parent_key)
                ...
                case NullableFlag(obj=obj1):
                    object_inline = _prepare_inline_flags(
                        obj1, ObjectDecoder(key, depth, set(), parent_key)
                    )
